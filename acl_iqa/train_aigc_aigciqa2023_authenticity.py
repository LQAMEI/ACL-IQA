...
checkpoint_dir = 'result/2023_aut_200epoch'
...
def train(model, best_result, best_epoch):
    with torch.cuda.amp.autocast(enabled=True):
        ...
        for sample_batched in loop:
            step += 1
            x, gmos, galign,gauth, prompt, image_name = sample_batched['I'], sample_batched['mos'], sample_batched['corr'], sample_batched['authenticity'], sample_batched['prompt'], sample_batched['image_name']
            x = x.to(device)
            gmos = gmos.to(device)
            galign = galign.to(device)
            gauth = gauth.to(device)
            ...
            logits_quality, moe_loss = model(x, input_texts, input_texts_se)
            ...
            total_loss = loss_m3(logits_quality, gauth.detach()).mean() + moe_loss
            
            auth_loss = loss_m3(logits_quality, gauth.detach()).mean()
        ...
        log_and_print(base_logger, f'auth loss: {auth_loss}')
...
def eval(loader, phase, dataset):
    ...
    for sample_batched in tqdm.tqdm(loader, desc='{}:{}'.format(dataset, phase)):
        ...
        x, gmos, prompt, image_name = sample_batched['I'], sample_batched['authenticity'],  sample_batched['prompt'], sample_batched['image_name']
        ...
        with torch.no_grad():
            logits_quality,_ = model(x, input_texts, input_texts_se)
...
for session in range(0,10):
    ...
    aigc_train_loader = set_dataset_aigc_2023(aigc_train_csv, bs, aigc_set, num_workers, preprocess3, train_patch, False)
    aigc_val_loader = set_dataset_aigc_2023(aigc_val_csv, bs , aigc_set, num_workers, preprocess2, test_patch, True)
