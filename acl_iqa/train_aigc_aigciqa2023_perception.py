...
checkpoint_dir = 'result/2023_qual_200epoch'
...
def train(model, best_result, best_epoch):
    with torch.cuda.amp.autocast(enabled=True):
        ...
        for sample_batched in loop:
            step += 1
            x, gmos, galign, prompt, image_name = sample_batched['I'], sample_batched['mos'], sample_batched['corr'],  sample_batched['prompt'], sample_batched['image_name']
            x = x.to(device)
            gmos = gmos.to(device)
            ...
            if logits_quality_col is not None:
                ...
                total_loss += 0.5*loss_m3(logits_quality_col, galign.detach()).mean()
            ...
            if logits_quality_adv is not None:
                ...
                total_loss += 0.5*loss_m3(logits_quality_adv, galign.detach()).mean()
            
            qual_loss = loss_m3(logits_quality, gmos.detach()).mean()
            col_loss =  0.5*loss_m3(logits_quality_col, galign.detach()).mean()
            adv_loss =  0.5*loss_m3(logits_quality_adv, galign.detach()).mean()
            moe_loss = moe_loss
