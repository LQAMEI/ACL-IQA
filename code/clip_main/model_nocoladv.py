...
class Adapter_MoElayer(nn.Module):
    ...
    def forward(self, gate_x, x, loss_coef=1):
        B, N, _ = x.shape  
        gate_x_global = torch.mean(gate_x,dim=1,keepdim=False)  
        gates, load = self.noisy_top_k_gating(gate_x_global, self.training) 
...
class MoEResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, text_d_model: int, n_head: int, attn_mask: torch.Tensor = None, adapter_topk=1):
        ...
        if self.adapter_k>0:
            self.adapter_MoE = Adapter_MoElayer(d_model,adapter_dim=8,k=self.adapter_k)
    ...
    def forward(self, x: torch.Tensor, past_key_values = None):
        ...
        x_adapter, adapter_loss = self.adapter_MoE(gates_x, hidden_states) 
        x_adapter = x_adapter.permute(1,0,2)  
        
        x = x + x_adapter + self.mlp(self.ln_2(x)) 
...
class UnimoModel(nn.Module):
    ...
    def encode(self, x: torch.Tensor, text: torch.Tensor):   
        x = self.conv1(x)  
        ...
        x, moe_loss = self.encoder(x, x_text)
    ...
    def forward(self, image, text1, sema, alpha=1):
        x, moe_loss  = self.encode(image, sema) 
        ...
        logits_per_image = logit_scale * x @ text1_features.t()

        return logits_per_image, moe_loss
...
def build_model(state_dict: dict, moe_layers=4, topk=1):
    ...
    new_state_dict = load_clip_weights(state_dict)
...
def load_clip_weights(state_dict):
    new_state_dict = {}
    ...
    mapping = {
        ...
        'visual.ln_post.bias': 'ln_post.bias',
