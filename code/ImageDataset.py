...
class AIGCDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 使用unfold方法将图像分割为高和宽均为kernel_h和kernel_w的小块。
        ...
        # 如果处于测试模式，则均匀选择self.num_patch个小块；否则随机选择
        if self.test:
            ...
            for i in range(self.num_patch):
                sel[i] = sel_step * i
            sel = sel.long()
            # 如果处于测试模式且blind为真，则设置MOS为0；否则，从数据集中获取MOS值。
            if self.blind:
                mos = 0.0
            else:
        ...
        # 调整到指定大小，上采样(双线插值)或下采样
        I_resized = F.interpolate(I, size=(kernel_h, kernel_w), mode='bilinear', align_corners=False)
        patches = torch.cat([patches, I_resized], dim=0) # 将patches与resize后的图片拼接，在patch维度多加一张原图
...
class AIGCDataset_3k(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #它会沿着高度、宽度方向提取出多个大小为 kernel_h的patches，每个片段之间相隔step个像素。
        patches = I.unfold(2, kernel_h, step).unfold(3, kernel_w, step).permute(2, 3, 0, 1, 4, 5).reshape(-1,
                                                                                                        n_channels,
                                                                                                        kernel_h,
                                                                                                        kernel_w)
        ...
        if self.test:
            ...
            align = float(self.data.iloc[index, 7])
            # mos = proc_label(mos)
        patches = patches[sel, ...]
        ...
        if self.data.shape[1] > 9:  
            model_index = self.data.iloc[index, 9]  # Assuming model_index is in the third column (index 2)  
            sample['category'] = model_index
...
class AIGCIQA2023Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        sample = {'I': patches, 'mos': mos, 'authenticity':authenticity, 'corr': corr,
                  'prompt':prompt, 'image_name':model_name}   # 对于AIGC2023返回的image_name实际为模型名
