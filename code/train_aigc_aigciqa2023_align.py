...
checkpoint_dir = 'result/2023_align_200epoch'
    ...
    with torch.cuda.amp.autocast(enabled=True):
        ...
        for sample_batched in loop:
            ...
            if logits_quality_col is not None:
                ...
                total_loss += 0.5*loss_m3(logits_quality_col, gmos.detach()).mean()
            ...
            if logits_quality_adv is not None:
                ...
                total_loss += 0.5*loss_m3(logits_quality_adv, gmos.detach()).mean()
            
            
            
            align_loss = loss_m3(logits_quality, galign.detach()).mean()
            col_loss =  0.5*loss_m3(logits_quality_col, gmos.detach()).mean()
            adv_loss =  0.5*loss_m3(logits_quality_adv, gmos.detach()).mean()
            moe_loss = moe_loss
